...
# @author stas, ethzoomer, akemiHomura-maow
...
MAX_POSITIONS: constant(uint256) = 400
...
@external
@view
def positions(_pool_addresses: DynArray[address, 200], _pool_types: DynArray[uint8, 200], _account: address)\
    -> DynArray[Position, MAX_POSITIONS]:
  """
  @notice Returns a collection of positions
  @param _account The account to fetch positions for
  @param _pool_addresses Pools to fetch positions from
  @param _pool_types Indicates the type of each corresponding pool 
  @return Array for Lp structs
  """
  positions: DynArray[Position, MAX_POSITIONS] = \
    empty(DynArray[Position, MAX_POSITIONS])
  
  # Saving a shorter pool list for cl pools only, for efficient lookup of cl positions
  cl_pools: DynArray[address, 200] = empty(DynArray[address, 200])
  for i in range(0, 200):
    if i >= len(_pool_addresses):
      break
    if _pool_types[i] == 1:
      cl_pools.append(_pool_addresses[i])
  ...
  for index in range(0, MAX_FACTORIES):
    ...
    if self._is_v2_factory(factory.address):
      ...
      for pindex in range(0, 200):
        if pindex >= len(_pool_addresses):
          break
        # Not a v2 pool, skip
        if _pool_types[pindex] != 0:
          continue
        pool_addr: address = _pool_addresses[pindex]
    ...
    if self._is_cl_factory(factory.address):
      ...
      for pindex in range(0, MAX_POSITIONS):
        ...
        if pos.lp != empty(address) and pos.lp in cl_pools:
          positions.append(pos)

      # fetch staked CL positions
      for pindex in range(0, 200):
        if pindex >= len(cl_pools):
          break
        pool_addr: address = cl_pools[pindex]
...
def _cl_position(_id: uint256, _account: address,\
  ...
  liquidity: uint256 = convert(data.liquidity, uint256)
...
def _v2_position(_account: address, _pool: address) -> Position:
  ...
  pos: Position = empty(Position)
  pos.lp = pool.address
  liquidity: uint256 = pool.balanceOf(_account)
  ...
  if claimable_delta0 > 0:
    pos.unstaked_earned0 += \
      (liquidity * claimable_delta0) / 10**convert(decimals, uint256)
  if claimable_delta1 > 0:
    pos.unstaked_earned1 += \
      (liquidity * claimable_delta1) / 10**convert(decimals, uint256)

  staked: uint256 = 0
  if gauge.address != empty(address):
    staked = gauge.balanceOf(_account)

  if liquidity + staked + pos.unstaked_earned0 == 0:
    return empty(Position)
  ...
  pos.amount0 = (liquidity * reserve0) / pool_liquidity
  pos.amount1 = (liquidity * reserve1) / pool_liquidity
  pos.staked0 = (staked * reserve0) / pool_liquidity
  pos.staked1 = (staked * reserve1) / pool_liquidity
